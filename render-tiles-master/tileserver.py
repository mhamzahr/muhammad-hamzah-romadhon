...
POSTGIS_TABLE = dict(
    ...
    password='12345',
    dbname='bali',
    table='(select ST_MPolyFromText(ST_AsText(geom)) as geom, name from bali) as bali')
LAYER_NAME = 'pantai'
...
def render_tile(layer, z, x, y):
    ...
    label = mapnik.TextSymbolizer(mapnik.Expression('[name]'), 'DejaVu Sans Book',24,mapnik.Color('red'
    ))
    label.halo_radius = 1
